...
ProjectService = {
  ...
  findAndSort : function(req, res){
    Project.aggregate([
      {$match: {}},
      {$unwind: {path: '$configurations', preserveNullAndEmptyArrays: true}},
      {$lookup: {from: 'configurations', localField: 'configurations', foreignField: '_id', as: 'config'}},
      ...
      {$project: {
          ...
          'view._id': 1,
          'view.centralPath': 1,
          'view.viewStats.totalViews': 1,
          'view.viewStats.viewsOnSheet': 1,
          'view.viewStats.createdOn': 1,
        }},
      {$unwind: {path: '$view.viewStats', preserveNullAndEmptyArrays: true}},
      {$sort: {'view.viewStats.createdOn': -1}},
      {$group: {
          _id:'$view._id',
          ...
          createdOn: {$first: '$view.viewStats.createdOn'},
          history: {$push: '$view.viewStats'}
        }
      },
      ...
      {$group: {
          ...
          viewsOnSheet: {$sum: '$viewsOnSheet'},
          mostRecent: {$max: '$createdOn'},
          history: {
            $push: {
