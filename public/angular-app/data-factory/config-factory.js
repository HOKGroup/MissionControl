...
function ConfigFactory($http){
    return {
        ...
        addFile: function addFile(configId, file) {
            return $http.post('/api/v1/configurations/' + configId + '/addfile', file).then(complete).catch(failed);
        },

        deleteFile: function deleteFile(configId, file) {
            return $http.post('/api/v1/configurations/' + configId + '/deletefile', file).then(complete).catch(failed);
        },
        ...
        getMany: function getMany(configIds) {
            return $http.post('/api/v1/configurations/getmany', configIds).then(complete).catch(failed);
        },
